...
2. **Move config** — centralize all env/config access into `config/index.ts`
3. **Move types** — extract all interfaces and types into `types/`
4. **Move utils** — extract pure helper functions into `utils/`
5. **Create repositories** — extract all DB queries from routes/services
6. **Create services** — extract all business logic from routes
7. **Create controllers** — extract request/response handling from routes
8. **Slim down routes** — routes should now be 5–10 lines each
9. **Update all imports** — fix every broken import path
10. **Run tests** — verify nothing broke (`pnpm test`)
11. **Run linter** — clean up any leftover issues (`pnpm lint`)
